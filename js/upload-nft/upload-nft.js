window.onload = function(e) {
    ...
    const INFERNA_SERVER_URL = 'http://3.101.57.15:3000'
        ...
        var uploadImage = function(fileName, blobFile) {                    
          $.ajax({
              ...
              success: function(response) {
                  ...
                  // have the client taken the result of reverse image search
                  let count = list.length ? list.length - 1 : 0;
                  if( count > 0 ) 
                    $('#search-list-notice').text('Oops, we have detected your NFT has already been used elsewhere. Please upload a unique file and try again')
                  else 
                    $('#search-list-notice').text('Great, there is no similar NFT anywhere')
              },
